  ...
  useEffect(() => {
    ...
    if (planToProcess && !loadingToCheck && !errorToCheck) {
      ...
      const updatedWorkout = {
        ...
        title: planName,
        createdAt: new Date().toISOString(),
        data: planToProcess // Store the full plan data
      };
      ...
      if (form) {
        ...
        if ('experienceLevel' in form && form.experienceLevel) {
          profileData.experienceLevel = form.experienceLevel;
        }
        if ('workoutDaysPerWeek' in form && form.workoutDaysPerWeek) {
          profileData.workoutDaysPerWeek = form.workoutDaysPerWeek;
        }
        if ('sessionDuration' in form && form.sessionDuration) {
          profileData.sessionDuration = form.sessionDuration;
        }
        
        // Handle array fields - ensure they're arrays, not empty strings
        if ('equipmentAccess' in form && form.equipmentAccess) {
          profileData.equipmentAccess = Array.isArray(form.equipmentAccess) 
            ? form.equipmentAccess 
            : [];
        }
        
        if ('dietaryRestrictions' in form && form.dietaryRestrictions) {
          profileData.dietaryRestrictions = Array.isArray(form.dietaryRestrictions)
        ...
        if ('foodPreferences' in form || 'preferences' in form) {
          ...
          if (prefs && Array.isArray(prefs) && prefs.length > 0) {
            profileData.foodPreferences = prefs;
          }
        }
        ...
        if ('dislikedFoods' in form) {
          ...
          if (dislikes && Array.isArray(dislikes) && dislikes.length > 0) {
            profileData.dislikedFoods = dislikes;
          }
        }

        if (upsertUserProfile) {
          console.log('💾 Saving user profile to Convex:', profileData);
      ...
      // Use planToProcess instead of ragPlan to support both RAG and integrated plans
      if (createWorkoutPlan && planToProcess && isOptimisticPlan && !alreadySaved && !isSaving) {
        ...
        const workoutPlanData = {
          ...
          phases: planToProcess.weeklyOutlines || [],
          fullPlanData: planToProcess, // ✅ Store complete AI-generated plan (required)
        };
