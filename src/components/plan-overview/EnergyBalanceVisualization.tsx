...
import { Flame, Utensils, Activity, Info } from 'lucide-react';
...
export function EnergyBalanceVisualization({ plan, userProfile, selectedWeek = 1 }: EnergyBalanceVisualizationProps) {
  ...
  const energyData = useMemo(() => {
    ...
    const exerciseCalories = calculateWeeklyExerciseCalories(weekData, weight);
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3 bg-white/40 p-4 rounded-xl border border-white/40">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="flex items-center gap-2 text-slate-700 font-medium">
                  <div className="p-1 rounded bg-orange-100 text-orange-600"><Activity className="w-3.5 h-3.5" /></div>
                  Exercise Burn
                </span>
                <span className="font-bold text-slate-800">-{Math.round(exerciseBurn.total / 7)} kcal</span>
              </div>
